    ...
    def analyse_map (self):
        for i in range(0, self.game.game_map.height - 1) :
            for j in range (0, self.game.game_map.height - 1) :
                if self.game.game_map[Position(i, j)].ship and \
                self.me.has_ship(self.game.game_map[Position(i, j)].ship.id) == False \
                and self.calc.get_closest_drop_dist(Position(i, j)) > 3 :
        ...
        self.data.max_radar = (-250 * self.game.turn_number / constants.MAX_TURNS + 300)  / self.data.nbr_player
