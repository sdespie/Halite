  ...
  #      return (5)
   # elif get_closest_drop_dist(ship, me, game_map) < (game.turn_number - MAX_T) + 5  and get_closest_drop_dist(ship, me, game_map) != 1:
    ...
    if get_closest_drop_dist(ship, me, game_map) == 0:
        ...
        ship_status[ship.id] = "returning"
        if get_closest_drop_dist(ship, me, game_map) >= MAX_T / 40 and me.halite_amount >= constants.DROPOFF_COST:
            if game.turn_number < 0.85 * MAX_T and check_halite_around(game_map, ship, me, game) > 14000:
                return (2)
            else :
                return (1)
        else:
